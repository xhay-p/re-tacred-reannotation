...
from pytorch_pretrained_bert.utils import constant, hierarchy
...
class DataProcessor(object):
    ...
    def get_train_examples(self, args):
        ...
        if args.filter_out:
            return self._create_examples(self.filter_instances(
    ...
    # @Akshay To filter out relation instances with ambiguous subject entity types
    def filter_instances(self, data):
        ...
        for eg in data:
            ...
            if (st, r, tt) in filter_out:
                continue
            else:
                new_dataset.append(eg)
        print("Total number of instances :: {}".format(len(new_dataset)))        
        return new_dataset

def convert_examples_to_features(examples, label2id, max_seq_length, tokenizer, special_tokens, relabel=False, mode='text'):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_mask += padding
        segment_ids += padding
        if relabel and example.label in constant.PARENTS:
            example.label = constant.PARENTS[example.label]
            rcount += 1
        ...
        features.append(
                InputFeatures(input_ids=input_ids,
                              ...
                              label_id=label_id))
    logger.info("Number of instances re-labeled: %d" % (rcount))        
...
def positive_accuracy(preds, labels):
    ...
    print("{} out of {} predictions are correct. Accuracy : {:.3%}".format(count, len(labels) - n_count, accuracy))
    return accuracy
...
def hierarchical_accuracy(preds, labels):
    ...
    # print("Count of correct negitive prediction :: {}".format(n_count))
    return h_accuracy, pred_dist
...
def evaluate(model, device, eval_dataloader, eval_label_ids, num_labels, verbose=True):
    ...
    h_result = compute_h_f1(preds, eval_label_ids.numpy())
    result['accuracy'] = accuracy(preds, eval_label_ids.numpy())
    result['positive_accuracy'] = positive_accuracy(preds, eval_label_ids.numpy())
    ...
    if verbose:
        logger.info("***** Hierarchical Eval results *****")
        for key in sorted(h_result.keys()):
            logger.info("  %s = %s", key, str(h_result[key]))
    if verbose:
        logger.info("***** Hierarchical Accuracy results *****")
        h, pd = hierarchical_accuracy(preds, eval_label_ids.numpy())
...
def main(args):
    ...
    opt = {}
    opt['hier_loss'] = args.hier_loss
    opt['lca'] = args.lca
    opt['hier_normalise'] = args.hier_normalise
    print('opt: ', opt)
    
    if args.filter_out:
        print("Filtering Initiating..!")
    if args.relabel:
        print("Relabeling Initiating..!")
    ...
    logger.info("  Num of relation labels = %d", num_labels)
    ...
    if args.do_eval:
        ...
        eval_features = convert_examples_to_features(
            eval_examples, label2id, args.max_seq_length, tokenizer, special_tokens, args.relabel, args.feature_mode)
        logger.info("***** Dev *****")
    ...
    if args.do_train:
        train_examples = processor.get_train_examples(args)
        train_features = convert_examples_to_features(
                train_examples, label2id, args.max_seq_length, tokenizer, special_tokens, args.relabel, args.feature_mode)
        ...
        for lr in lrs:
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_batches):
                    ...
                    loss = model(input_ids, segment_ids, input_mask, label_ids, opt)
                    if n_gpu > 1:
                        loss = loss.mean()
                    if args.gradient_accumulation_steps > 1:
                        ...
                        if save_model:
                            ...
                            if best_result:
                                ...
                                with open(output_eval_file, "w") as writer:
                                    for key in sorted(result.keys()):
                                        writer.write("%s = %s\n" % (key, str(result[key])))
                print("Epoch  ::  {}; Train loss  ::  {}; Dev F1 ::  {}".format(epoch, tr_loss / nb_tr_steps, result[args.eval_metric]))
                dev_f1.append(result[args.eval_metric])
                train_loss.append(tr_loss / nb_tr_steps)
                dev_prec.append(result['precision'])
                dev_recall.append(result['recall'])

    if args.do_eval:
        if args.eval_test:
            eval_examples = processor.get_test_examples(args)
            eval_features = convert_examples_to_features(
                eval_examples, label2id, args.max_seq_length, tokenizer, special_tokens, args.relabel, args.feature_mode)
            logger.info("***** Test *****")
            logger.info("  Num examples = %d", len(eval_examples))
    ...
    pickle.dump(dct ,open(os.path.join(args.output_dir, "train_detail.pkl"), "wb"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filter_out', action='store_true', help='Remove setences with ambiguous subject entity type from data.')
    parser.add_argument('--relabel', action='store_true', help='Hierarchical Re-labelling: Mapping finer positive relations to coarser relation from hierarchy.')
    ...
    parser.add_argument('--lca', action='store_true', help='True uses lca based distance for hierarchical distance loss, False uses simple distance.')
    parser.add_argument('--hier_normalise', action='store_true', help="Normalise Hierarchical Distance loss")
