...
from hierarchy_utils import hierarchy, constant
...
class HierarchicalDistanceLoss(torch.nn.Module):
    ...
    def get_hierarchical_dist(self, pred, gold):
        if self.id2label[gold] == 'no_relation' and self.id2label[pred] != 'no_relation':
            ...
            d = self.DIS_MATRIX[gold, pred]

        if self.normalise:
            return (d / np.sum(self.DIS_MATRIX[gold,:len(self.id2label)])) * len(self.id2label)
        elif self.log_dist:
            return np.log(d)
        else:
            return d
