...
from utils.constant import LABEL_TO_ID_HIER
...
def get_lca(G, source, target):
    if isinstance(source, int) and isinstance(target, int):
        ...
        path1 = nx.shortest_path(G, LABEL_TO_ID_HIER[source], LABEL_TO_ID_HIER['root'])
        path2 = nx.shortest_path(G, LABEL_TO_ID_HIER[target], LABEL_TO_ID_HIER['root'])
    flag = 0
    for p in path1:
        for q in path2:
            if p == q:
...
def common_path_score(G, label, pred):
    common_nodes = []
    path1 = nx.shortest_path(G, LABEL_TO_ID_HIER[label], LABEL_TO_ID_HIER['root'])
    # print(path1)
    path2 = nx.shortest_path(G, LABEL_TO_ID_HIER[pred], LABEL_TO_ID_HIER['root'])
    # print(path2)
    for p in path1[::-1]:
        for q in path2[::-1]:
            if p == q:
                common_nodes.append(p)
    # print(common_nodes)
    lca = get_lca(G, label, pred)
    # print(lca, nx.shortest_path(G, lca, LABEL_TO_ID_HIER['root']))
    score = 1 / len(path1) * len(common_nodes)
    # print(score)
    return score, nx.shortest_path_length(G, LABEL_TO_ID_HIER[label], lca)
